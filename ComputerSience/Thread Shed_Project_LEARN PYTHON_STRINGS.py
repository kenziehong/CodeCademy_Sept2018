...
for element in daily_transactions_split:
  element_clean=[]
  for item in element:
    element_clean.append(item.replace("\n","").strip())
  transactions_clean.append(element_clean)  
...
#11.Now, iterate through transactions_clean and for each transaction: Append the customers name to customers. Append the amount of the sale to sales. Append the threads sold to thread_sold
...
for element in thread_sold:
  thread_sold_split.append(element.split("&"))
...
for element in colors:
  print("Thread Shed sold {} threads of {} thread today".format(color_count(element), element))
